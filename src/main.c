...
// CreateFile(const char *filename, bool overwrite);
...
// -> Write all content to a file 					                                    = void
// WriteAll(const char *content[], const char *path);

// -> Write a single line of content to a file  	                              = void
// WriteLine(int line, const char *content, const char *path);

// -> Append content to end of file 				                                    = void
// AppendLine(const char *content, const char *path);

// -> Read a single line of content specified in file                           = char*
// ReadLine(const char *path, int line);

// -> Read all content from file 					                                      = char*
...
int main() {
  ...
  printf("\nRead a line from %s\n->%s", myFile->finalPath, ReadLine(2, myFile->finalPath));
